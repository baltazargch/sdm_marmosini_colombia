...
# lapply(OCCS, function(x) { dir.create(paste0(save.dir, gsub(" ", "_", x[1,1]))) })
...
# for( i in seq_along(OCCS)){
  ...
  # sp.files <- all.files[grep(sp, all.files)]
  # sp.tables <- sp.files[grep('tables/fit', sp.files)]
  ...
  for(row in 1:nrow(sp.tuned)){
    ...
    if(file.exists(paste0(save.dir, '/', sp, '_', 
      ...
      # write_sf(r_modif, paste0(to.save, '.shp'), overwrite=T)
      # writeRaster(rm, paste0(to.save, '.grd'), overwrite=T)
